"""Faithfulness evaluation: completeness residuals + deletion curves."""
...
__all__ = ["faithfulness", "scorecard"]
...
def deletion_curve(explainer: Explainer, query, candidate, level: str | None = None) -> dict:
    """Delete top-attributed dims from the query in order; score should fall fast.

    Returns per-step scores plus the AUC (lower = more faithful) vs a random-order baseline.
    """
